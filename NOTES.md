...
## 2026-02-26 작업 (4차) — 으쌰으쌰 정렬 변경 + "다른 게시글 가져오기" 버그 수정

### 커밋: `48614c8`

### A. 으쌰으쌰 sortOrder 변경

"🔥 으쌰으쌰" 소분류를 게시판 대분류의 가장 좌측(첫 번째)으로 이동.

| 변경 전 | 변경 후 |
|---------|---------|
| sortOrder: 8 (마지막) | sortOrder: 1 (첫 번째) |

- 나머지 소분류 sortOrder 2~8로 재정렬
- 파일: `data/categories.ts`

### B. 버그 수정: "다른 게시글 가져와서 고치기" 빈 상태 문제

**증상**: 글쓰기 → 으쌰으쌰 선택 → "다른 게시글 가져와서 고치기" → "같은 카테고리에 다른 글이 없어요" 표시

**원인**: `fetchOtherPosts()`가 소분류(`categoryMinorSlug`)로만 검색. 으쌰으쌰(id:48)는 mock 포스트 0건이라 항상 빈 결과. 대분류 폴백 로직 부재.

**수정 내용 (3건)**:

| 파일 | 변경 |
|------|------|
| `app/write/page.tsx` | `fetchOtherPosts()` — 소분류 글 0건 시 같은 대분류 전체에서 폴백 검색 |
| `app/write/page.tsx` | Sheet 제목 — 폴백 시 "같은 대분류의 다른 글"로 변경 |
| `data/posts.ts` | 으쌰으쌰 mock 포스트 2건 추가 (서울대 p48, KAIST p49) |

**폴백 동작 흐름**:
1. 같은 소분류에서 글 검색 (기존 동작)
2. 글이 있으면 표시
