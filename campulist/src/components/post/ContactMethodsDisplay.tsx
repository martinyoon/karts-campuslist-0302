...
export default function ContactMethodsDisplay({ contactMethods }: Props) {
  ...
  return (
    <div className="mt-2 rounded-xl border border-border p-2"> {/* 간격 압축: mt-4 → mt-2, p-3.5 → p-2 */}
      <p className="mb-1.5 text-sm font-medium">연락 방법</p> {/* 간격 압축: mb-2.5 → mb-1.5 */}
      <div className="space-y-1"> {/* 간격 압축: space-y-2 → space-y-1 */}
        {contactMethods.phone && (
          <div className="flex items-center gap-1.5 text-sm">
            ...
            <a href={`tel:${contactMethods.phone}`} className="font-medium text-green-600 hover:underline">{contactMethods.phone}</a>
        ...
        {contactMethods.kakaoLink && (
          <a
            ...
            className="flex items-center gap-1.5 text-sm transition-colors hover:text-yellow-600"
          >
        ...
        {contactMethods.email && (
          <div className="flex items-center gap-1.5 text-sm">
            ...
            <a href={`mailto:${contactMethods.email}`} className="font-medium hover:underline">{contactMethods.email}</a>
            <button
              onClick={() => { navigator.clipboard.writeText(contactMethods.email!); toast('이메일이 복사되었습니다'); }}
