...
export default function IconToggle() {
  ...
  if (!mounted) return <Button variant="ghost" className="flex h-auto flex-col items-center gap-0.5 px-2 py-1" />;
  ...
  return (
    <Button
      ...
      aria-label={show ? '아이콘 숨기기' : '아이콘 표시'}
    >
      {show ? (
      ...
      <span className="text-xs font-medium">{show ? '아이콘' : '아이콘'}</span>
    </Button>
  );
}
